...
function render() {
    ...
    const yOffset = 5.0;
    const yAngularSpeedDeg = 45.0; // 45 deg / sec
    const yAngularSpeedRad = glMatrix.toRadian(yAngularSpeedDeg);

    let theta = glMatrix.toRadian(cameraCircleSpeed * elapsedTime);
    let camX = radius * Math.sin(theta);
    let camZ = radius * Math.cos(theta);
    let camY = yOffset + yAmplitude * Math.sin(yAngularSpeedRad * elapsedTime);
